      ...
      (fun i c ->
        let copy = function
          | None ->
              None
          | Some cl ->
              Some {cl with lits = Array.copy cl.lits}
        in
        a.(i) <- copy c)
      p ;
    a
...
*)
  ...
  let enqueue st p reason =
    ...
    ( if !debug then
      match reason with
      | Some r ->
          Format.eprintf "Applying rule %a@." (print_rule st) r
      | _ ->
          () ) ;
  ...
  let assume st p = raise_level st ; enqueue st p None
  ...
  let reset st =
    ...
    for i = 0 to Array.length st.st_refs - 1 do
      st.st_refs.(i) <- 0 ; st.st_pinned.(i) <- false
    done ;
  ...
  let rec find_next_lit st =
    ...
    | p :: rem ->
        ...
        if st.st_seen_var.(var_of_lit p) = st.st_seen then (
          ...
          undo_one st p ; (p, reason) )
        else (undo_one st p ; find_next_lit st)
...
*)
          ...
          if st.st_cur_level = 0 then (
            ...
            let assignment =
              ...
              for v = 0 to Array.length st.st_assign - 1 do
                ...
                | True ->
                    acc := lit_of_var v true :: !acc
                | False ->
                    acc := lit_of_var v false :: !acc
                | Unknown ->
                    ()
              done ;
              !acc
  ...
  let rec solve_aux ?callback st x =
    ...
    try
      ...
      if s st then (protect st ; true) else solve_aux st ?callback x
    with Conflict _ ->
  ...
  let add_rule st lits reasons =
    ...
    if not !is_true then
      ...
      | 0 ->
          assert false
      | 1 ->
          add_un_rule st all_lits lits.(0) reasons
      | 2 ->
          add_bin_rule st all_lits lits.(0) lits.(1) reasons
      | _ ->
  ...
  let stats st =
    let (t, f, u) =
      Array.fold_left
        (fun (t, f, u) -> function True -> (t + 1, f, u) | False ->
              (t, f + 1, u) | Unknown -> (t, f, u + 1))
        (0, 0, 0)
        st.st_assign
    in
