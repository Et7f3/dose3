...
let pp_version fmt pkg =
  try
    Format.fprintf
      fmt
      "%s"
      (decode (Cudf.lookup_package_property pkg "number"))
...
let get_property prop pkg =
  ...
  with Not_found -> warning "%s missing" prop ; raise Not_found
...
let who_provides univ (pkgname, constr) =
  ...
  let filter = function
    | (p, None) ->
        Some p
    | (p, Some v) when Cudf.version_matches v constr ->
        Some p
    | _ ->
        None
  in
...
let cone universe pkgs =
  ...
  List.iter
    (fun pkg -> Queue.add (Cudf.uid_by_package universe pkg) queue)
    pkgs ;
