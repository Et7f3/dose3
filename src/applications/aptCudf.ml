...
module Options = struct
  ...
  add
    options
    ~long_name:"dump"
    ~help:"dump the cudf universe and solution"
    dump ;
...
let parse_solver_spec filename =
  ...
  ( try
    ...
    with
  | Sys_error _ ->
      fatal "cannot parse CUDF solver specification %s" filename
  | End_of_file ->
      ()
        ...
        let p =
          Hashtbl.fold
            (fun cudffield (edspfield, plain, compiledre) acc ->
              ...
              with
              | None ->
                  acc (* field not found in this edsp package *)
              | Some fieldvalue -> (
                  let matches =
...
(*        warning "Possibly duplicated package (same version, name and architecture) : (%s,%s,%s)"
  ...
  *)
  ...
  if OptParse.Opt.get Options.dump then (
    ...
    close_out oc ) ;
  (* do nothing. *)
  if OptParse.Opt.get Options.noop then (info "Noop & Exit." ; exit 0) ;
