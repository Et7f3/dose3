...
let main () =
  ...
  let deb_load_list options ?(status = []) sources urilist =
    ...
    let archs =
      if native <> "" then native :: options.Dose_debian.Debcudf.foreign
      else []
    in
    let dll =
      List.map
    ...
    let pkglist =
      if status = [] then pkglist
      else Dose_debian.Packages.merge status pkglist
    in
    let origsourcelist =
    ...
    in
    let tables =
      Dose_debian.Debcudf.init_tables ~options (srclist @ pkglist)
    in
    let from_cudf (p, i) =
      Dose_debian.Debcudf.get_real_version tables (p, i)
    in
    let to_cudf (p, v) =
      ...
      List.map
        (fun l ->
          List.map
            (fun debpkg ->
              let cudfpkg =
                Dose_debian.Debcudf.tocudf tables ~options debpkg
              in
              let id = (cudfpkg.Cudf.package, cudfpkg.Cudf.version) in
