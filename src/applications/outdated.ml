...
module Options = struct
  ...
  add
    options
    ~long_name:"dump"
    ~help:"Dump the cudf package list and exit"
    dump
end
...
let evalsel getv target constr =
  ...
  match target with
  | `Hi v ->
      evalsel (getv v + 1) constr
  | `Lo v ->
      evalsel (getv v - 1) constr
  | `Eq v ->
      evalsel (getv v) constr
  | `In (_, v2) ->
      evalsel (getv v2 - 1) constr
...
let main () =
  ...
  let result =
    outdated ~summary ~failure ~explain ~dump ~options packagelist
  in
