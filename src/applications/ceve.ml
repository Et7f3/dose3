    ...
    (fun l ->
      ...
      | "cnf" ->
          Printf.fprintf
            oc
            "%s"
            (Depsolver.output_clauses ~enc:Depsolver.Cnf u)
      | "dimacs" ->
          Printf.fprintf
            oc
            "%s"
      ...
      | ("deb" | "debsrc") as t ->
          ...
          | None ->
              fatal
                "If -T is deb or debsrc, then the raw list must not be None" ) ;
          let cudf2deb cudfpkg =
            let id = (cudfpkg.Cudf.package, cudfpkg.Cudf.version) in
            try Hashtbl.find cudftodeb_table id
            with Not_found ->
              fatal
                "cannot find cudf package the mapping - is the input deb:// \
                 or debsrc://?"
          in
          if t = "deb" then
            List.iter
      ...
      | ("dot" | "gml" | "grml") as t -> (
          ...
          | ("pkg" | "strdeps" | "conj" | "dom") as gt -> (
              let g =
                match gt with
                | "pkg" ->
                    Defaultgraphs.PackageGraph.dependency_graph u
                | "strdeps" ->
              ...
              | _ ->
                  fatal "Option -T %s not supported together with -G %s" t gt )
          | s ->
              fatal "Opation %s not supported" s )
      | t ->
          fatal "-T %s format unknown." t ) ;
      close_out oc)
    [plist]
