...
let challenged
  ...
  let cluster_iter (sn,sv) l =
    List.iter (fun (_version,realversion,cluster) ->
  ...
  let tables = Dose_debian.Debcudf.init_tables ~step:2 ~versionlist repository in
  let getv v = Dose_debian.Debcudf.get_cudf_version tables ("",v) in
  let pp = pp tables in
  ...
  let results =
    ...
    map (fun ((sn,sv,version),(cluster,vl,constr)) ->
       ...
       *)
      let discr = Dose_debian.Evolution.discriminant ~bottom:true (evalsel getv) vl constr in
      ...
      List.iter (function
        ...
        | (target,equiv) when not(downgrades) &&
            (lesser_or_equal getv target equiv version) ->
              debug "Target: %s" (Dose_debian.Evolution.string_of_range target);
        ...
        | (target,equiv) ->
            ...
            let migrationlist = Dose_debian.Evolution.migrate cluster target in
...
let main () =
  ...
  List.iter (fun (((sn,sv,version),(target,equiv)),broken) ->
    ...
    Format.printf "target: %s@." (Dose_debian.Evolution.string_of_range target);
