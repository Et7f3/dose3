...
module InputOptions = struct
  ...
  let parse_cmdline (it, im) posargs =
    let add_format t =
      List.map (fun s -> Url.scheme_to_string t ^ "://" ^ s)
    in
    let fg = OptParse.Opt.get foreground in
    let bg = OptParse.Opt.get background in
    let fg =
      (if List.length (posargs @ fg) = 0 then ["-"] else posargs) @ fg
    in
    ...
    if List.length default > 0 then (
      ...
      if List.mem "compare" default then
        add
          ...
          ~long_name:"compare"
          ~help:
            "When used with a pef input type, selects a comparison function"
          compare )
...
module DistribOptions = struct
  ...
  let set_default_options = function
    ...
    | `Pef ->
        Some (Pef Dose_debian.Debcudf.default_options)
    | `Opam ->
        Some (Opam Dose_opam.Opamcudf.default_options)
    | _ ->
        None

  let set_options = function
    | `Deb | `DebSrc ->
        Some (Deb (set_deb_options ()))
    | `Edsp ->
        Some (Edsp (set_deb_options ()))
    | `Pef ->
        Some (Pef Dose_debian.Debcudf.default_options)
    | `Opam ->
        Some (Opam (set_opam_options ()))
    | _ ->
        None
    ...
    if List.length default > 0 then (
      ...
      if List.mem "deb-drop-b-d-indep" default then
        add
          ...
          ~help:
            "Drop the Build-Depends-Indep field from source packages (build \
             no Architecture:all packages)"
          deb_drop_bd_indep ;
  ...
  let add_option ?group ?short_name ?long_name ?help options v =
    ...
    match group with
    | None ->
        add options ?short_name ?long_name ?help v
    | Some group ->
        add options ~group ?short_name ?long_name ?help v
end
