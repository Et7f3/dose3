...
let opam_load_list ?options file =
  ...
  in
  let cl =
    List.flatten (List.map (Dose_opam.Opamcudf.tocudf ~options tables) pkglist)
  in
  let preamble = Dose_opam.Opamcudf.preamble in
  let request =
    Dose_opam.Opamcudf.requesttocudf tables (Cudf.load_universe cl) request
  in
  (preamble, [cl; []], request, from_cudf, to_cudf, None, [])
...
let cudf_load_list file =
  let (preamble, pkglist, request) =
    ...
    | (None, pkglist, Some req) ->
        (Cudf.default_preamble, pkglist, req)
    | (Some p, pkglist, None) ->
        (p, pkglist, Cudf.default_request)
    | (Some p, pkglist, Some req) ->
        (p, pkglist, req)
  in
...
let opam_parse_input ?options urilist =
  match urilist with
  | [[p]] when unpack `Opam p = "-" ->
      fatal "no stdin for opam yet"
  | [[p]] ->
...
let cudf_parse_input urilist =
  match urilist with
  | [[p]] when unpack `Cudf p = "-" ->
      fatal "no stdin for cudf yet"
  | [[p]] ->
...
let edsp_parse_input options urilist =
  match urilist with
  | [[p]] when unpack `Edsp p = "-" ->
      fatal "no stdin for edsp yet"
  | [[p]] ->
...
let parse_input ?(options = None) ?(raw = false) ?compare urilist =
  ...
  | (`Edsp, _) ->
      edsp_parse_input Dose_debian.Debcudf.default_options filelist
  | (`Opam, _) ->
      opam_parse_input filelist
  | (`Npm, _) ->
      npm_parse_input filelist
  (* |`Opam, Some (StdOptions.Opam options) -> opam_parse_input ~options filelist *)
  | (`Pef, Some (StdOptions.Pef _)) ->
      pef_parse_input ?compare filelist
  | (`Csw, None) ->
      csw_parse_input filelist
  | (`Hdlist, None) ->
      fatal "hdlist Not supported."
  | (`Synthesis, None) ->
      fatal "synthesis input format not supported."
  | (s, _) ->
      fatal "%s Not supported" (Url.scheme_to_string s)
