...
let compose_raw (major, minor, patch, pre, build) =
  let str =
    ...
    | ("", "", "") ->
        Printf.sprintf "*"
    | (_, "", "") ->
        Printf.sprintf "%s" major
    | (_, _, "") ->
        Printf.sprintf "%s.%s" major minor
    | (_, _, _) ->
        Printf.sprintf "%s.%s.%s" major minor patch
  in
  ...
  | ([], []) ->
      str
  | (l, []) ->
      Printf.sprintf "%s-%s" str (str_pre l)
  | ([], l) ->
      Printf.sprintf "%s+%s" str (str_build l)
  | (lp, lb) ->
      Printf.sprintf "%s-%s+%s" str (str_pre lp) (str_build lb)
    ...
    | s -> (
      ...
      with Failure _ ->
        ...
        raise
          (Invalid_argument
             (Printf.sprintf
                "%s: Conversion Error: \"%s\" cannot be converted to an integer"
                composed
                s)) )
  in
