...
let parse_req = Dose_pef.Packages.lexbuf_wrapper Dose_pef.Packages_parser.vpkglist_top
  ...
  try
    {
      ...
      dist_upgrade = Dose_pef.Packages.parse_s ~default:false Dose_pef.Packages.parse_bool "dist_upgrade" par;
      switch = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_string "switch" par;
      switches = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_string_list "switches" par;
      profiles = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_string_list "profiles" par;
...
class package ?(name=("package",None)) ?(version=("version",None)) ?(depends=("depends",None))
  ...
  inherit Dose_pef.Packages.package ~name ~version ~depends ~conflicts ~provides ~recommends:depopts ~extras par

  val switch : (string * string list) =
    let p = Dose_pef.Packages.parse_string_list ~rex:Dose_pef.Packages.comma_regexp in
...
empty *)
let parse_package_stanza ((switch,switches,_profiles) as options) ?(extras=[]) par =
  try
    let pkg_switch =
      let p = Dose_pef.Packages.parse_string_list ~rex:Dose_pef.Packages.comma_regexp in
    ...
    if not (List.mem "all" pkg_switch ||
      ...
      raise (Dose_pef.Packages.IgnorePackage (
        Printf.sprintf
      ...
      let pkg =
        let depends =
          let f = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_builddepsformula in
        ...
        in
        let conflicts =
          let f = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_builddepslist in
      ...
      Some pkg
  with
  |Dose_pef.Packages.IgnorePackage s -> begin
      let n = Dose_pef.Packages.parse_s ~default:"?" Dose_pef.Packages.parse_name "package" par in
      let v = Dose_pef.Packages.parse_s ~default:"?" Dose_pef.Packages.parse_version "version" par in
