...
let parse_depends =
  ...
  [
    ...
    ( "depends filter arch",
      [("depends", (Common.Format822.dummy_loc, "a [arch]"))],
...
let version_lag =
  let (request, packagelist) =
    Opam.Packages.input_raw "tests/opam/version_lag.opam"
  in
  let options =
    {
  ...
  in
  let universe =
    Opam.Opamcudf.load_universe ~options Version.compare packagelist
  in
...
let opamcudf_filter =
  let (_, packagelist) =
    Opam.Packages.input_raw "tests/opam/filter_universe.opam"
  in
  let function_to_test (switch, switches) =
    let options =
      {Opam.Opamcudf.default_options with Opam.Opamcudf.switch; switches}
    in
