...
let label =  __label ;;
include Util.Logging(struct let label = "dose_opam.opamcudf" end) ;;
...
let add_extra extras tables (switch,activeswitch) pkg =
  ...
  in
  let version_lag =
    let l = Hashtbl.find tables.Dose_pef.Pefcudf.versions_table pkg#name in
    let count,i =
  ...
  in
  let depopts = ("depopts", `Vpkgformula (Dose_pef.Pefcudf.loadll tables pkg#depopts)) in
    ...
    if List.mem "all" pkg#switch || List.mem switch pkg#switch then
      ...
      let cudfpkg =
        { Cudf.default_package with
          ...
          version = Dose_pef.Pefcudf.get_cudf_version tables (pkg#name,pkg#version);
          installed = List.mem switch pkg#installedlist;
          keep;
          depends = Dose_pef.Pefcudf.loadll tables ~arch:switch ~archs:all_switches depends;
          conflicts = Dose_pef.Pefcudf.loadlc tables ~arch:switch ~archs:all_switches pkg#conflicts;
          provides = Dose_pef.Pefcudf.loadlp tables ~arch:switch ~archs:all_switches pkg#provides;
