...
class source ?(name=("Package",None)) ?(version=("Version",None))
  ...
  val name : (string * Dose_pef.Packages_types.name) =
    let f = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_name in
    Dose_pef.Packages.get_field_value ~parse:f ~par ~field:name

  val version : (string * Dose_pef.Packages_types.version) =
    let f = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_version in
  ...
  val architecture : (string * Dose_pef.Packages_types.architecture list) =
    let f = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_archlist in
  ...
  val build_conflicts_indep : (string * Dose_pef.Packages_types.builddepslist) =
    let f = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_builddepslist in
    Dose_pef.Packages.get_field_value ~parse:f ~par ~field:build_conflicts_indep

  val build_conflicts_arch : (string * Dose_pef.Packages_types.builddepslist) =
    let f = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_builddepslist in
    Dose_pef.Packages.get_field_value ~parse:f ~par ~field:build_conflicts_arch
