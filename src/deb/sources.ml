...
let matchprofile profiles = function
  | [] ->
      true
  | ll ->
      List.exists (List.for_all (fun (c, p) -> c = List.mem p profiles)) ll
...
let src2pkg ?(dropalternatives = false) ?(profiles = []) ?(noindep = false)
  ...
  let dropalt l =
    ...
    | [p] ->
        [p]
    | hd :: _ ->
        List.filter (fun p -> fst p = fst hd) l
  in
  let depends ll =
    List.filter_map
      (fun l ->
        ...
        | [] ->
            None
        | l ->
            Some (if dropalternatives then dropalt l else l))
      ll
  in
  let extras_profiles =
    match profiles with
  ...
  new Packages.package
    ...
    ~conflicts:
      ( "",
        Some
          (conflicts (conflicts_indep @ srcpkg#build_conflicts @ conflicts_arch))
      )
...
let srcbin_table universe =
  ...
  let aux binpkg =
    if CudfAdd.get_property "type" binpkg = "bin" then
      ...
      with
      | NotfoundSrc ->
          () (* this binary is not associated to any src *)
      | MismatchSrc sl ->
