...
let label =  __label ;;
include Util.Logging(struct let label = "dose_deb.edsp" end) ;;
...
let from_apt_request arch request = function
  |Apt.Install vpkgreqlist ->
      List.fold_left (fun acc -> function
        |(Some Dose_pef.Packages_types.I, ((n,None),c), _) -> {acc with install = ((n,arch),c) :: acc.install}
        |(Some Dose_pef.Packages_types.R, ((n,None),c), _) -> {acc with remove =  ((n,arch),c) :: acc.remove}
        |(None, ((n,None),c), _) -> {acc with install = ((n,arch),c) :: acc.install}

        |(Some Dose_pef.Packages_types.I, vpkg, _) -> {acc with install = vpkg :: acc.install}
        |(Some Dose_pef.Packages_types.R, vpkg, _) -> {acc with remove = vpkg :: acc.remove}
...
let parse_req (label,(loc,s)) =
  let aux v = Dose_pef.Packages.lexbuf_wrapper Dose_pef.Packages_parser.vpkg_top (label,v) in
...
let parse_request_stanza par =
  ...
  let request = Dose_pef.Packages.parse_s ~required:true parse_edsp_version "Request" par in
  {
    request = request;
    ...
    upgrade = Dose_pef.Packages.parse_s ~default:false Dose_pef.Packages.parse_bool "Upgrade" par;
    architecture = Dose_pef.Packages.parse_s ~default:None Dose_pef.Packages.parse_string_opt "Architecture" par;
    architectures = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_string_list "Architectures" par;
    distupgrade = Dose_pef.Packages.parse_s ~default:false Dose_pef.Packages.parse_bool "Dist-Upgrade" par;
    autoremove = Dose_pef.Packages.parse_s ~default:false Dose_pef.Packages.parse_bool "Autoremove" par;
    strict_pin = Dose_pef.Packages.parse_s ~default:true Dose_pef.Packages.parse_bool "Strict-Pinning" par;
    preferences = Dose_pef.Packages.parse_s ~default:"" Dose_pef.Packages.parse_string "Preferences" par;
    cmdline = Dose_pef.Packages.parse_s ~default:"" Dose_pef.Packages.parse_string "Command-Line" par;
  }
;;
...
let parse_apt_id = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_string
let parse_apt_pin = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_int_s
