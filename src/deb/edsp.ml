...
let from_apt_request arch request = function
      ...
      List.fold_left
        (fun acc -> function
          ...
          | (Some Dose_pef.Packages_types.R, vpkg, _) ->
              {acc with remove = vpkg :: acc.remove} | (None, vpkg, _) ->
              {acc with install = vpkg :: acc.install})
        request
        vpkgreqlist
  | Apt.Remove vpkgreqlist ->
      List.fold_left
        ...
        vpkgreqlist
  | Apt.Upgrade _ ->
      {request with upgrade = true}
  | Apt.DistUpgrade _ ->
      {request with distupgrade = true}

let parse_req (label, (loc, s)) =
  let aux v =
    Dose_pef.Packages.lexbuf_wrapper
      Dose_pef.Packages_parser.vpkg_top
      (label, v)
  in
...
let parse_edsp_version (label, (_, s)) =
  ...
  | ["EDSP"; s] when float_of_string s >= 0.4 ->
      s
  | _ ->
      raise (Format822.ParseError ([], label, "Invalid EDSP version."))
...
let rec packages_parser ?(request = false) (req, acc) p =
  ...
  | Some stanza when req.strict_pin = false -> (
    ...
    | None ->
        assert false (* this is not possible in this branch *)
    | Some st ->
        packages_parser (req, st :: acc) p )
  | _ ->
      assert false
