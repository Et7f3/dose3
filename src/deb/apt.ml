...
let parse_req label s =
  ...
  Dose_pef.Packages.lexbuf_wrapper Dose_pef.Packages_parser.request_top (label,(_loc,s))
...
let parse_pref_package (_,(_,s)) =
  ...
  else Pref.Package (Dose_pef.Packages.parse_name ("apt pref",(Format822.dummy_loc,s)))
...
let parse_preferences_stanza par =
  {
    Pref.package = Dose_pef.Packages.parse_s ~required:true parse_pref_package "Package" par;
    pin = Dose_pef.Packages.parse_s ~required:true parse_pin "Pin" par;
    pin_priority = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_int "Pin-Priority" par;
  }
