...
let parse_popcon s =
  match Pcre.split ~rex:blank_regexp s with
  | rank :: name :: inst :: _ ->
      (int_of_string rank, name, int_of_string inst)
  | _ ->
      fatal "Parse error %s\n" s
...
let parse_request_apt s =
  ...
  | "install" :: tl ->
      Install (List.map (parse_pkg_req !suite) tl)
  | "remove" :: tl ->
      Remove (List.map (parse_req "apt req remove") tl)
  | ["upgrade"] ->
      Upgrade !suite
  | ["dist-upgrade"] ->
      DistUpgrade !suite
  | _ ->
      fatal "Bad apt request '%s'" s
...
let parse_pref_labels s =
  List.map
    (fun s' ->
      ...
      | [v] when Pcre.pmatch ~rex:di_regexp v ->
          ("v", v)
      | [v] when Pcre.pmatch ~rex:al_regexp v ->
          ("a", v)
      | [l; v] ->
          (l, v)
      | _ ->
          fatal "To many '=' in label %s" s)
...
let parse_pin (_, (_, s)) =
  try
    ...
    | "version" ->
        Pref.Version (Pcre.get_substring substrings 2)
    | "origin" ->
        Pref.Origin (Pcre.get_substring substrings 2)
    | s ->
