...
let add_name_arch n = function
  | "" ->
      CudfAdd.encode n
  | a ->
      CudfAdd.encode (Printf.sprintf "%s:%s" n a)
...
let add_arch_info ?(native_arch = "") ?(package_arch = "") = function
  | (n, Some "any") ->
      CudfAdd.encode n
  | (n, Some "native") ->
      add_name_arch n native_arch
  | (n, Some a) ->
      add_name_arch n a
  | (n, None) ->
      add_arch native_arch n package_arch
  ...
  List.flatten
    (List.map
       (fun (((name, _) as vpkgname), constr) ->
         ...
         match constr with
         | None ->
             [(vencname, Some (`Eq, Util.max32int - 1))]
         | Some ("=", v) ->
  ...
  if not (Option.is_none options.native) then
    ...
    let package_arch =
      if Sources.is_source pkg then
        ...
        | Some host_arch ->
            host_arch (* source package : build deps on the cross arch *)
                ...
                let l =
                  ...
                  *)
                      ...
                      | (true, false) ->
                          [((n, a), c)] (* real conflict *)
                      | (true, true) ->
                          [] (* self conflict on real package, drop it *)
                      | (false, false) -> (
                        match c with
                        | None ->
                            [((n, a), None)] (* virtual conflict *)
                        | _ ->
                            []
