...
let range ?(bottom = false) vl =
  ...
  let rec aux acc = function
    | (None, []) ->
        acc
    | (None, a :: t) ->
        aux (`Hi a :: acc) (Some a, t)
    | (Some b, a :: t) ->
        aux (`In (a, b) :: `Eq b :: acc) (Some a, t)
    | (Some b, []) when bottom = false ->
        `Eq b :: acc
    | (Some b, []) ->
        `Lo b :: `Eq b :: acc
  in
...
let add_unique h k v =
  ...
  with Not_found ->
    ...
    Hashtbl.add vh v () ; Hashtbl.add h k vh
...
let align version target =
  ...
  | Version.NonNative ("", _, _, _) | Version.Native ("", _, _) ->
      target
