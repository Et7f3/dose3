    ...
    | Some fn ->
        ...
        let aux line =
          if line.[0] = '#' || not (String.contains line '\t') then ()
            ...
            | _ ->
                fatal "Cannot parse debtuple: %s" debtuple
        in
        List.iter aux (Std.input_list ic) ;
        close_in ic
    | None ->
...
let src_matches_arch alias real =
  ...
  if alias = real || alias = "any" || alias = "all" then true
    ...
    let alias =
      ...
      | ["any"; libc; os; cpu] ->
          Some ("any", libc, os, cpu)
      | [abi; "any"; os; cpu] ->
          Some (abi, "any", os, cpu)
      | [abi; libc; "any"; cpu] ->
          Some (abi, libc, "any", cpu)
      | [abi; libc; os; "any"] ->
          Some (abi, libc, os, "any")
      | ["any"; os; cpu] ->
          Some ("any", "any", os, cpu)
      | [libc; "any"; cpu] ->
          Some ("any", libc, "any", cpu)
      | [libc; os; "any"] ->
          Some ("any", libc, os, "any")
      | ["any"; cpu] ->
          Some ("any", "any", "any", cpu)
      | [os; "any"] ->
          Some ("any", "any", os, "any")
      | ["any"] ->
