...
let parse_source v = Dose_pef.Packages.lexbuf_wrapper Dose_pef.Packages_parser.source_top v
let parse_binarylist v = Dose_pef.Packages.lexbuf_wrapper Dose_pef.Packages_parser.vpkglist_top v
...
class package ?(name=("Package",None)) ?(version=("Version",None)) ?(depends=("Depends",None))
    ...
    ?(extras=([],None)) par = object(_)

  inherit Dose_pef.Packages.package ~name ~version ~depends ~conflicts ~provides ~recommends ~extras par

  val architecture : (string * Dose_pef.Packages_types.architecture) =
    let parse = Dose_pef.Packages.parse_s ~required:true Dose_pef.Packages.parse_string in
  ...
  val essential : (string * bool) =
    let parse = Dose_pef.Packages.parse_s ~default:false Dose_pef.Packages.parse_bool in
  ...
  val priority : (string * string) =
    let parse = Dose_pef.Packages.parse_s ~default:"" Dose_pef.Packages.parse_string in
    Dose_pef.Packages.get_field_value ~parse ~par ~field:priority

  val pre_depends : (string * Dose_pef.Packages_types.vpkgformula) =
    let parse = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_vpkgformula in
  ...
  val breaks : (string * Dose_pef.Packages_types.vpkglist) =
    let parse = Dose_pef.Packages.parse_s ~default:[] Dose_pef.Packages.parse_vpkglist in
...
let parse_package_stanza filter archs extras par =
  ...
  |Dose_pef.Packages.IgnorePackage s -> begin
      ...
      let a = Dose_pef.Packages.parse_s ~default:"?" Dose_pef.Packages.parse_string "Architecture" par in
...
let parse_packages_in ?filter ?(archs=[]) ?(extras=[]) fname ic =
  ...
  try
    ...
    Format822.parse_from_ch (Dose_pef.Packages.packages_parser fname stanza_parser) ic
