...
module Make (G : Sig.I) = struct
  ...
  let averageDegree graph = computeDegree graph ; Option.get !_avgdegree
  ...
  let scatteredPlotIn graph = computeDegree graph ; Option.get !_indata

  let scatteredPlotOut graph = computeDegree graph ; Option.get !_outdata
