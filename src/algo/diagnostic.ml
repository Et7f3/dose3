...
let reason map universe =
  ...
  List.filter_map (function
      | DependencyInt (i, _vl, _il) when i = globalid ->
          None
      ...
      | ConflictInt (i, j, _vpkg) when i = globalid || j = globalid ->
          fatal
            "the package encoding global constraints can't be in conflict \
             (uid %d - %d)"
            i
            j
      ...
      | MissingInt (i, vl) ->
          Some (Missing (from_sat (map#inttovar i), vl))
          ...
          List.filter_map
            (function
              | i when i = globalid ->
                  None
              | i ->
                  Some {(from_sat (map#inttovar i)) with Cudf.installed = true})
            (f_int ~all ()))
  | FailureInt f ->
      Failure (fun () -> reason map universe (f ()))
...
module ResultHash = OcamlHash.Make (struct
  ...
  let equal v w =
    match (v, w) with
    | (Missing (_, v1), Missing (_, v2)) ->
        v1 = v2
    | (Conflict (i1, j1, _), Conflict (i2, j2, _)) ->
        i1 = i2 && j1 = j2
    | _ ->
        false
...
let condense_graph gr =
  ...
  let getlist =
    List.filter_map (function
        | PkgV.Pkg {value = p; root = false} as v ->
            Some (p, v)
        | _ ->
            None)
  in
          ...
          PMap.iter
            (fun (_name, sl, pl) l ->
              ...
              | [] ->
                  ()
              | [(_, hd)] ->
                  Hashtbl.add h hd (List.hd l, sl, pl)
              | pkgl ->
                  let vpid =
...
let print_error ?(condense = false) ?(minimal = false) pp root fmt l =
  ...
  let get_package v =
    match v with
    | DG.PkgV.Pkg {DG.value = p; _} ->
        [p]
    | DG.PkgV.Set s ->
        CudfAdd.Cudf_set.elements s
    | _ ->
        raise Not_found
  in
  ...
  let pp_reason_conflicts fmt (vi, vj, vpkg) =
    ...
    if not minimal then (
      let pl1 =
        try fst (DJ.shortest_path gr vroot vi) with Not_found -> []
      in
      let pl2 =
        try fst (DJ.shortest_path gr vroot vj) with Not_found -> []
      in
      if pl1 <> [] then
  ...
  Defaultgraphs.SyntacticDependencyGraph.G.iter_edges_e
    (fun (src, label, dst) ->
      match !label with
      | DG.PkgE.Conflict vpkg ->
          conflicts := (src, dst, vpkg) :: !conflicts
      | DG.PkgE.MissingDepends vpkgs ->
          missing := (src, vpkgs) :: !missing
      | _ ->
          ())
    gr ;
  ...
  let l =
    ResultHash.fold
      (fun k v acc ->
        ...
        | Missing (_, _) ->
            result.unique_missing <- result.unique_missing + 1
        | _ ->
            () ) ;
