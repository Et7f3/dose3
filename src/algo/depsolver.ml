...
let edos_coinstall_prod ?(global_constraints = []) universe ll =
  ...
  let rec permutation = function
    | [] ->
        return []
    | h :: t ->
        bind (permutation t) (fun t1 -> List.map (fun h1 -> h1 :: t1) h)
  in
...
let is_consistent univ =
  ...
  | ((true | false), _) ->
      fatal "Bug in Cudf_checker.is_consistent"
...
let check_request_using ?call_solver ?dummy ?(explain = false)
  ...
  | (Some call_solver, None) -> (
    try
      ...
      Sat (presol, sol)
    with
    | CudfSolver.Unsat when not explain ->
        Unsat None
...
let depclean ?(global_constraints = []) ?(callback = fun _ -> ()) universe
            ...
            List.map
              (fun vpkg ->
                ...
                | [] ->
                    (vpkglist, vpkg, [], [])
                | l ->
                    (vpkglist, vpkg, before @ [[vpkg]] @ after, l))
              vpkglist
          in
  ...
  let test_conflict l =
    List.fold_left
      (fun acc -> function (vpkg, []) -> (vpkg, []) :: acc | (_, _) -> acc
  ...
  let test_depends univ (`CudfPool (_, pool)) pkg l =
    List.fold_left
      (fun acc -> function (vpkglist, vpkg, _, []) ->
            (vpkglist, vpkg, []) :: acc | (vpkglist, vpkg, depends, l) ->
