...
module SyntacticDependencyGraph = struct
  ...
  module PkgV = struct
    ...
    let compare x y =
      ...
      | (Or i1, Or i2) ->
          i1 - i2
      | (Pkg {value = p1; _}, Pkg {value = p2; _}) ->
          CudfAdd.compare p1 p2
      | (Set s1, Set s2) ->
          CudfAdd.Cudf_set.compare s1 s2
      | (_, _) ->
          Stdlib.compare x y
  ...
  module DotPrinter = struct
    module Display = struct
      ...
      let graph_attributes _ =
        [`Rankdir `LeftToRight (* ; `Concentrate true *)]
      ...
      let vertex_attributes v =
        ...
        | PkgV.Set _ ->
            [`Label "empty??"; `Shape `Record]
        | PkgV.Missing _ ->
            [`Label "Missing"; `Color 0x00FF00; `Shape `Ellipse]
    ...
    let print fmt g = fprint_graph fmt g ; Format.fprintf fmt "@."
  end
  ...
  let dependency_graph ?root univ =
    ...
    in
    let timer =
      Util.Timer.create "SyntacticDependencyGraph.dependency_graph"
    in
...
module ActionGraph = struct
  module PkgV = struct
    ...
    let hash = function
      | Install p ->
          Hashtbl.hash (1, p.Cudf.package, p.Cudf.version)
      | Remove p ->
          Hashtbl.hash (0, p.Cudf.package, p.Cudf.version)
...
module PackageGraph = struct
  ...
  let conjdepgraph_int ?(transitive = false) graph univ p =
    ...
    List.iter
      (fun vpkgs ->
        match CudfAdd.resolve_deps univ vpkgs with
        | [q] when not (CudfAdd.equal q p) ->
            add_edge ~transitive graph p q
        | _ ->
            ())
      p.Cudf.depends
  ...
  let dependency_graph_aux conjunctive gr universe pkg =
    ...
    List.iter
      (fun vpkgs ->
        ...
        | [p] ->
            G.add_edge gr pkg p
        | l when not conjunctive ->
            List.iter (G.add_edge gr pkg) l
        | _ ->
            ())
      pkg.Cudf.depends
      ...
      (fun id pkg ->
        ...
        List.iter
          (fun vpkgs ->
            ...
            | [p] ->
                G.add_edge graph id p
            | l when not conjunctive ->
                List.iter (G.add_edge graph id) l
            | _ ->
                ())
          pkg.Cudf.depends)
      universe ;
        ...
        List.iter
          (fun vpkgs ->
            ...
            | [i] when not (Hashtbl.mem visited i) ->
                Queue.add i queue ; G.add_edge graph id i
