...
let test_conjunctive_dependency_closure =
  "dependency closure conjunctive"
  >:: fun _ ->
  List.iter
    (fun pkg ->
      (*Printf.eprintf "%s\n%!" (CudfAdd.string_of_package pkg);*)
      let dcl =
        Depsolver.dependency_closure ~conjunctive:true universe [pkg]
      in
...
let test_conj_dependency =
  "conjunctive dependency closure"
  >:: fun _ ->
