...
let pefvpkg to_cudf vpkgname =
  let constr n = function
    | None ->
        None
    | Some (op, v) ->
        Some (pefcudf_op op, snd (to_cudf (n, v)))
  in
  match vpkgname with
  | ((n, None), c) ->
      (CudfAdd.encode n, constr n c)
  | ((n, Some ("any" | "native")), c) ->
      (CudfAdd.encode n, constr n c)
  | ((n, Some a), c) ->
      (CudfAdd.encode (n ^ ":" ^ a), constr n c)
...
let get_cudf_version tables (package, version) =
  try
    ...
    Hashtbl.replace
      tables.reverse_table
      (CudfAdd.encode package, i + 1)
      version ;
    i + 1
  with Not_found ->
    warning
      "Cannot find cudf version for pef package %s (= %s)"
      package
      version ;
    raise Not_found
...
let get_real_version tables (p, i) =
  let n =
    try fst (ExtString.String.split p "%3a") with Invalid_string -> p
  in
