...
let parse_s ?default ?(required = false) f label par =
  ...
  with Not_found -> (
    ...
    | (false, None) ->
        raise Not_found (* for extra labels *)
    | (true, None) ->
...
let parse_bool (label, (_, s)) =
  ...
  | "Yes" | "yes" | "True" | "true" ->
      true
  | "No" | "no" | "False" | "false" ->
      false (* this one usually is not there *)
  | s ->
      raise (Format822.Type_error (label ^ " - wrong value : " ^ s))
  ...
  List.filter_map
    (fun (label, p) ->
      try
        match p with
        | None ->
            Some (label, parse_s parse_string label par)
        | Some parse_f ->
            Some (label, parse_f label par)
      with Not_found -> None)
    extras
...
let packages_parser fname stanza_parser p =
  let rec packages_parser_aux fname stanza_parser acc p =
    ...
    | Some stanza -> (
      ...
      | None ->
          packages_parser_aux fname stanza_parser acc p
      | Some st ->
          packages_parser_aux fname stanza_parser (st :: acc) p )
  in
